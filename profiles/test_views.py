...
class TestViews(TestCase):
    ...
    def test_get_edit_profile_page(self):
        ...
        login = self.client.login(username='Tester', password='testerpass')
        response = self.client.get(
            reverse('edit_profile', kwargs={'user': self.user}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_profile_post(self):
        ...
        response = self.client.post(reverse(
            'edit_profile', kwargs={'user': self.user}), {
                'user': self.user,  'name': 'Name Updated',
                'email': 'updatedemail@test.com'})
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, reverse('user_profile', kwargs={'user': self.user}))
    ...
    def test_delete_profile(self):
        ...
        response = self.client.delete(
            reverse('delete_profile', kwargs={'user': self.user}))
        self.assertEqual(response.status_code, 302)
